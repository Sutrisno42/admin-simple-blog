...
function checkAuth(req) {
  // return true
  let checker = permissions.value.filter((e) => {
    let res = e.name == req;
    if (res == true) {
    ...
    <template v-if="layoutType === 'vertical' || layoutType === 'semibox'">
      <ul class="navbar-nav h-100" id="navbar-nav">
        ...
        <li v-for="menu in data.menus" :key="menu.name" class="nav-item">
          ...
          >
            <i :class="menu.icon"></i>
            <span style="font-size: 13px">{{ menu.display_name }}</span>
          </a>
          <router-link
            v-if="!menu.caret"
            v-show="checkAuth(menu.name)"
                  ...
                  <ul class="nav nav-sm flex-column">
                    ...
                    >
                      <router-link
                        ...
                        v-show="checkAuth(grandChild.name)"
                        class="nav-link"
                        data-key="t-simple-page"
                      >
